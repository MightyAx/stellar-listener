...
class SenseHatObserver:
    def __init__(self, sense: SenseHat, transformer: OrientationTransformer, screen_rotation):
        self.sense = sense
    ...
    def __adjust_observation(self, orientation):
        if (self.screen_rotation in [0, 180]):
            return self.__roll_as_pitch(orientation, self.screen_rotation == 180)
        return self.__pitch_as_pitch(orientation, self.screen_rotation == 270)
    
    def __pitch_as_pitch(self, orientation, flip=False):
        yaw = (orientation['yaw'] + self.screen_rotation - 90) % 360
